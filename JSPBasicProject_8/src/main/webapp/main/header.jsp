<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    /*
    		session
    		클래스 : HttpSession = > 생성 ㅇ:request.getSession()
    		=>내장 객체 사용이 가능 
    		=> 서버에 저장 =>데이터 노출이 없다 (보안이 뛰어나다)
    		=>주요 기능 = >데이터 일부정보를 저장후에 사용
    	    ...
    	    2.읽기 : getAttribute("키");
    	    		=>형변환
    	    3.저장기간  : setMaxinactionInterVal()
    	    								====초단위
    	    		=> 기본 default:1800초 (30분)
    	    4. 삭제 
    	    	=> 일부 삭제:removeAttribure(키) => 장바구니 취소
    	    	=> 전체 삭제:invalidate()=> 로그아웃 
    	    5.session은 브라우저당 1개만 배정 (접속자 마다 한개만 배정)
    	    			===== 구분 : id => getId()
    	    							========채팅
  ...
  <header id="header" class="clear"> 
    ...
    <div class="fl_right">
      ...
      <form method=post action="../member/login.jsp" class="inline">
        ...
        <li><%=name %>(<%=admin.equals("y")?"관리자":"일반사용자" %>)님 로그인되었습니다.</li>
    ...
    <ul class="clear">
      ...
      <% if (id==null){ %>
        ...
        <ul>
          ...
          <li><a href="pages/full-width.html">수정</a></li>
          <li><a href="pages/sidebar-left.html">탈퇴</a></li>
        </ul>
        <%} %>
      </li>
