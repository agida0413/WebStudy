...
public class BoardListServlet extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      */
      /*
         데이터를 전송 
         => 전송없이 수행 => GET (주로 HTML 화면 출력시 사용)
         => GET => DEFAULT => doGet
         
         <input> = 사용자가 입력해서 서버로 전송할 때 사용 
      */
      // 전송해야되는 브라우저 설정 
      PrintWriter out=response.getWriter();
      // 데이터 받기 (DAO 에서 받아서 HTML 에 출력)
      //BoardListServlet?page=1 = > get 방식 
      ...
      List<BoardVO> list=dao.boardListData(curpage); // 1페이지 
      ...
      */
      ...
      out.println("</select>");
      out.println("<input type=text name=ss size=20>"); // 텍스트 입력칸
      out.println("<input type=button value=검색>"); // 검색창 
      out.println("</td>");
      out.println("<td align=right>");
      out.println("<a href=BoardListServlet?page="+(curpage>1?curpage-1:curpage)+">이전</a>&nbsp");
      out.println(curpage+"page / "+totalpage+"page");
      out.println("<a href=BoardListServlet?page="+(curpage<totalpage?curpage+1:curpage)+">다음</a>&nbsp");
      out.println("</td>");
      out.println("</tr>");
      out.println("</table>");
      out.println("</center>");
