...
public class GoodsDAO {
   ...
   private String[] tables= {"","goods_all","goods_special",
         "goods_best","goods_new"}; // 하나의 메소드 => 타입번호 (테이블별로 사용할수있음) => 각 테이블의 pk 다름 => order by로 정렬
   ...
   public static GoodsDAO newInstence()
   {
      if(dao==null)
         dao=new GoodsDAO();
      return dao;
   ...
   // 목록 출력
   public List<GoodsVO> goodsListData(int page,int type) // type = tables배열의 순서
   {
      ...
      try
      {
         ...
         // setString(1,"goods_all") ==> 'goods_all'
